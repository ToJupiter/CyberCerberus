...
class Spider:
    ...
    async def _fetch_single(self, session: aiohttp.ClientSession, domain: str, classification: str) -> Dict[str, Any]:
        async with self.concurency_limit:
            ...
            try:
                async with session.get(target_url, headers=self.headers, ssl=False) as response:
    ...
    async def process_csv(self, csv_file_path: str, filter_csv_path: str = "filters.csv", batch_size: int = 1000, max_iterations: int = 5):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=self.timeout) as session:
            ...
            for iteration in range(max_iterations):
                ...
                for i in tqdm.tqdm(range(0, len(tasks), batch_size), total=total_batches, desc=f"Iter {iteration+1}"):
...
def main():
    ...
    parser.add_argument('--concurrency', type=int, default=256, help='Maximum concurrent connections')
    parser.add_argument('--timeout', type=int, default=15, help='Request timeout in seconds')
    parser.add_argument('--batch_size', type=int, default=1000, help='Number of domains to process per batch')
    parser.add_argument('--max_iterations', type=int, default=5, help='Maximum number of crawling iterations')
    ...
    logger.info(f"Max iterations: {args.max_iterations}, Batch size: {args.batch_size}")
